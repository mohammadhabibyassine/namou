...
export function OrdersList() {
  ...
  return (
    <div className="namou-container py-7 sm:py-10">
      ...
      <div
        className="border-line bg-surface mt-7 overflow-x-auto rounded-xl border p-1.5"
        role="group"
        aria-label="Filter orders by status"
      >
        <div className="flex min-w-max items-center gap-1">
          <span className="text-subtle hidden px-3 font-mono text-[9px] font-semibold uppercase sm:block">
            Status
          </span>
          <span
            ...
            aria-hidden="true"
          />
          <div className="grid min-w-[48rem] flex-1 grid-cols-6 gap-1">
            {filters.map((filter) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                  className={cn(
                    "inline-flex min-h-10 items-center justify-center gap-2 rounded-lg px-4 font-mono text-[9px] uppercase transition-all",
                    active
      ...
      {query.isPending ? (
        ...
        <div className="hairline-panel mt-4 p-10 text-center">
          ...
          <Link
            href="/shop"
            className="action-button mt-5 inline-block rounded-lg px-5 py-3 font-mono text-[10px] uppercase"
          >
            Enter the shop
          </Link>
        </div>
